...
app.get("/productss", function (_req, res) {
  const sql = "SELECT p.*, b.statusborrow FROM product p LEFT JOIN borrow b ON p.id = b.id WHERE p.statusproduct = 0";
...
app.put("/borrows/:borrowid", function (req, res) {
  ...
  con.query(sql, [status, borrowid], function (err, results) {
    ...
    if (results.affectedRows != 1) {
      console.error("Row updated is not 1");
      return res.status(500).json({ error: "Update failed: No or multiple rows affected" });
    }
